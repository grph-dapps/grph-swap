...
function contract({privateKey, api, version, net, contractAddress}) {
  ...
  const code = readFileSync(resolve(process.cwd(), "scilla/contracts", "GraphToken.scilla"), "utf8");
  let initParams = [{"vname":"contract_owner","type":"ByStr20"},{"vname":"total_mint","type":"Uint128"},{"vname":"name","type":"String"},{"vname":"symbol","type":"String"},{"vname":"decimals","type":"Uint32"},{"vname":"init_supply","type":"Uint128"}];
  ...
  return {
    ...
    async deploy(contract_owner, total_mint, name, symbol, decimals, init_supply, gasLimit = 60000) {
    ...
    async IncreaseAllowance(spender, amount, gasPrice = 2000,gasLimit = 2000, zilAmount = 0, callback) {
        ...
        return confirmedTxn.receipt.success === true;
    },async Mint(to, amount, gasPrice = 2000,gasLimit = 2000, zilAmount = 0, callback) {
        const args = arguments;
        const e = new Error();
        const frame = e.stack.split("\n")[1];
        const tag = frame.split(" ")[5].split(".")[1];
        ...
        return confirmedTxn.receipt.success === true;
    },async Burn(amount, gasPrice = 2000,gasLimit = 2000, zilAmount = 0, callback) {
        const args = arguments;
        const e = new Error();
        const frame = e.stack.split("\n")[1];
        const tag = frame.split(" ")[5].split(".")[1];
        ...
        return confirmedTxn.receipt.success === true;
    },async SetMinter(new_minter, gasPrice = 2000,gasLimit = 2000, zilAmount = 0, callback) {
        const args = arguments;
        const e = new Error();
        const frame = e.stack.split("\n")[1];
        const tag = frame.split(" ")[5].split(".")[1];
        const params = [{"vname":"new_minter","type":"ByStr20"}].map((param, index) => {
            if (typeof args[index] === "object") {
                param.value = args[index];
            } else {
                param.value = typeof args[index] === "boolean" ? {constructor: args[index] ? "True" : "False", argtypes: [], arguments: []} : args[index].toString();
        ...
        return confirmedTxn.receipt.success === true;
    },async RemoveMinter(minter, gasPrice = 2000,gasLimit = 2000, zilAmount = 0, callback) {
        const args = arguments;
        const e = new Error();
        const frame = e.stack.split("\n")[1];
        const tag = frame.split(" ")[5].split(".")[1];
        const params = [{"vname":"minter","type":"ByStr20"}].map((param, index) => {
            if (typeof args[index] === "object") {
                param.value = args[index];
            } else {
                param.value = typeof args[index] === "boolean" ? {constructor: args[index] ? "True" : "False", argtypes: [], arguments: []} : args[index].toString();
        ...
        return confirmedTxn.receipt.success === true;
    },async CloseMint(gasPrice = 2000,gasLimit = 2000, zilAmount = 0, callback) {
        const args = arguments;
        const e = new Error();
        const frame = e.stack.split("\n")[1];
        const tag = frame.split(" ")[5].split(".")[1];
    ...
    events: {"RemoveMinter":{"name":"RemoveMinter","params":[{"name":"minter","type":"ByStr20"}]},"SetMinter":{"name":"SetMinter","params":[{"name":"minter","type":"ByStr20"}]},"TransferFromSuccess":{"name":"TransferFromSuccess","params":[{"name":"initiator","type":"ByStr20"},{"name":"sender","type":"ByStr20"},{"name":"recipient","type":"ByStr20"},{"name":"amount","type":"Uint128"}]},"TransferSuccess":{"name":"TransferSuccess","params":[{"name":"sender","type":"ByStr20"},{"name":"recipient","type":"ByStr20"},{"name":"amount","type":"Uint128"}]},"DecreasedAllowance":{"name":"DecreasedAllowance","params":[{"name":"token_owner","type":"ByStr20"},{"name":"spender","type":"ByStr20"},{"name":"new_allowance","type":"Uint128"}]},"IncreasedAllowance":{"name":"IncreasedAllowance","params":[{"name":"token_owner","type":"ByStr20"},{"name":"spender","type":"ByStr20"},{"name":"new_allowance","type":"Uint128"}]},"Minted":{"name":"Minted","params":[{"name":"minter","type":"ByStr20"},{"name":"recipient","type":"ByStr20"},{"name":"amount","type":"Uint128"}]},"Burnt":{"name":"Burnt","params":[{"name":"burner","type":"ByStr20"},{"name":"burn_account","type":"ByStr20"},{"name":"amount","type":"Uint128"}]}},
    fields: {"total_supply":{"total_supply":"total_supply","Uint128":"Uint128"},"balances":{"balances":"balances","Map (ByStr20) (Uint128)":"Map (ByStr20) (Uint128)"},"allowances":{"allowances":"allowances","Map (ByStr20) (Map (ByStr20) (Uint128))":"Map (ByStr20) (Map (ByStr20) (Uint128))"},"minters":{"minters":"minters","Map (ByStr20) (Bool)":"Map (ByStr20) (Bool)"},"total_mint_amount":{"total_mint_amount":"total_mint_amount","Uint128":"Uint128"},"allowed_total_amount":{"allowed_total_amount":"allowed_total_amount","Uint128":"Uint128"},"mintable":{"mintable":"mintable","Bool":"Bool"}}
  };  
};
